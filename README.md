...
"TerminalGui.config": {
  ...
  "scripts": {
    "bash": [
      "PROJECT_TYPE=\"\"",
      "TG_LANG_CACHE_FILE=\".vscode/terminal-gui.temp/react_lang.txt\"",
      "",
      "tg_lang() {",
      "  local start=\"${1:-$PWD}\"",
      "  local cache=\"$TG_LANG_CACHE_FILE\"",
      "  local pt=\"\"",
      "",
      ...
      "  if [ -z \"$pt\" ]; then",
      "    local cur=\"$start\"",
      "    local root; root=$(git -C \"$start\" rev-parse --show-toplevel 2>/dev/null || echo \"/\")",
      "    while :; do",
      "      if compgen -G \"$cur/tsconfig*.json\" >/dev/null; then pt=ts; break; fi",
      "      if [ -f \"$cur/package.json\" ] && grep -qiE '\"typescript\"|\"@types/react\"|@typescript-eslint' \"$cur/package.json\"; then pt=ts; break; fi",
      "      [ \"$cur\" = \"$root\" ] && break",
      "      local parent; parent=$(dirname \"$cur\"); [ \"$parent\" = \"$cur\" ] && break; cur=\"$parent\"",
      "    done",
      "    if [ -z \"$pt\" ]; then",
      "      if compgen -G \"$start/*.tsx\" >/dev/null || compgen -G \"$start/*.ts\" >/dev/null; then pt=ts; else pt=js; fi",
      "    fi",
      "  fi",
      "",
      "  # ensure folder + cache file always written",
      "  mkdir -p \".vscode/terminal-gui.temp\" 2>/dev/null",
      "  echo \"$pt\" > \"$cache\"",
      "",
      "  # map + session var",
      "  PROJECT_TYPE=\"$pt\"",
      "  if [ \"$pt\" = ts ]; then TG_LANG=ts; JSX_EXT=tsx; HOOK_EXT=ts; SERVICE_EXT=ts",
      "  else TG_LANG=js; JSX_EXT=jsx; HOOK_EXT=js; SERVICE_EXT=js",
      "  fi",
      "}",
      "",
      "detect_ts_js_cached() {        # keep name; delegate to tg_lang and always write cache",
      "  tg_lang \"${1:-$PWD}\"",
      "}",
      "",
      "# Capitalize",
      ...
      "}",
      "",
      "# (optional standalone detector still here; fixed /dev/null typo)",
      "detect_ts_js() {",
      "  local start_dir=\"${1:-$PWD}\"",
      "  TG_LANG=js; JSX_EXT=jsx; HOOK_EXT=js; SERVICE_EXT=js",
      "  local git_root; git_root=$(git -C \"$start_dir\" rev-parse --show-toplevel 2>/dev/null || echo \"/\")",
      "  local cur=\"$start_dir\"",
      "  while :; do",
      ...
      "  fi",
      "  if [ \"$TG_LANG\" = ts ]; then JSX_EXT=tsx; HOOK_EXT=ts; SERVICE_EXT=ts",
      "  else JSX_EXT=jsx; HOOK_EXT=js; SERVICE_EXT=js",
      "  fi",
      "}",
      "",
      "tg_css_to_scss() {",
      ...
      "  local rc=\"$1\" ig=\"$2\"PROJECT_TYPE=\"\"",
      "TG_LANG_CACHE_FILE=\"\"",
      "",
      "tg_lang() {",
      "  local root=\"${1:-$PWD}\"",
      "  TG_LANG_CACHE_FILE=\"$root/.vscode/terminal-gui.temp/react_lang.txt\"",
      "",
      "  local pt=\"\"",
      "  [ -f \"$TG_LANG_CACHE_FILE\" ] && pt=$(tr -d '\\r\\n' < \"$TG_LANG_CACHE_FILE\")",
      "",
      "  if [ -z \"$pt\" ]; then",
      "    if compgen -G \"$root/tsconfig*.json\" >/dev/null; then pt=ts",
      "    elif [ -f \"$root/package.json\" ] && grep -qiE '\"typescript\"|\"@types/react\"|@typescript-eslint' \"$root/package.json\"; then pt=ts",
      "    elif compgen -G \"$root/*.tsx\" >/dev/null || compgen -G \"$root/*.ts\" >/dev/null; then pt=ts",
      "    else pt=js",
      "    fi",
      "    mkdir -p \"$root/.vscode/terminal-gui.temp\" 2>/dev/null",
      "    echo \"$pt\" > \"$TG_LANG_CACHE_FILE\"",
      "  fi",
      "",
      "  PROJECT_TYPE=\"$pt\"",
      "  if [ \"$pt\" = ts ]; then TG_LANG=ts; JSX_EXT=tsx; HOOK_EXT=ts; SERVICE_EXT=ts",
